...
export default class Lista_produtos {
  ...
  static exibirProduto(item, marcaItem, status_modal, produto_selecionado, fecharModal) {
  return (
    <View style={style.image_text} key={item.id}>
      <CheckBox
        checked={item.selecionado}
        onPress={() => marcaItem(item.id)}
      />
      <Image 
        style={style.image} 
        resizeMode="contain" 
              ...
              <View style={style.modal_fundo}>
                <View style={style.modal} key={produto_selecionado.id}>
                  <Image
                    style={style.image}
                    resizeMode="contain"
                    source={produto_selecionado.imagem}
                  />
                  <View style={style.caixaFundo}>
                  <View>               
                  ...
                  <Pressable
                    style={[style.botoes, { paddingHorizontal: "10%", marginVertical: 6 }]}
                    onPress={fecharModal}
                  >
